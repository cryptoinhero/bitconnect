...
bool  ComputeNextStakeModifier(const CBlockIndex* pindexPrev, uint64_t &nStakeModifier, bool& fGeneratedStakeModifier)
{
    ...
    for (int nRound=0; nRound<min(64, (int)vSortedByTimestamp.size()); nRound++)
    {
        ...
        if (gArgs.GetBoolArg("-debug", false) && gArgs.GetBoolArg("-printstakemodifier", false))
            LogPrintf("ComputeNextStakeModifier: selected round %d stop=%s height=%d bit=%d, nStakeModifierNew=%016llx\n",
                nRound, DateTimeStrFormat(nSelectionIntervalStop), pindex->nHeight, pindex->GetStakeEntropyBit(), nStakeModifierNew);
    }
...
bool CheckStakeKernelHash(unsigned int nBits, CBlockIndex* pindexPrev, const CBlockHeader& blockFrom, unsigned int nTxPrevOffset, const CTransactionRef& txPrev, const COutPoint& prevout, unsigned int nTimeTx, uint256& hashProofOfStake, bool fPrintProofOfStake)
{
    ...
    int64_t nTimeWeight = min((int64_t)nTimeTx - (int64_t)txPrev->nTime - params.nStakeMinAge, params.nStakeMaxAge);
...
unsigned int GetStakeModifierChecksum(const CBlockIndex* pindex)
{
    ...
    ss << pindex->nFlags << (pindex->IsProofOfStake() ? pindex->hashProofOfStake : uint256S("0"))  << pindex->nStakeModifier;
...
// bitconnect: entropy bit for stake modifier if chosen by modifier
