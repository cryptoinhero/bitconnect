 ...
 * between bitconnectd, and bitconnect-qt and non-server tools.
*/
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/bitconnectgithub/bitconnectcoin>";
    const std::string URL_WEBSITE = "<https://bitconnect.com/>";
...
void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("bitconnect-loadblk");
...
/** Sanity checks
 *  Ensure that Bitconnect is running in a usable environment with all
...
{
    // Make sure only a single Bitconnect process is using the data directory.
...
bool AppInitSanityChecks()
{
    ...
    // bitconnect: init hash seed
    bitconnectRandseed = GetRand(1 << 30);

#ifdef ENABLE_CHECKPOINTS
    // bitconnect: moved here because ECC need to be initialized to execute this
    if (gArgs.IsArgSet("-checkpointkey")) // bitconnect: checkpoint master priv key
    {
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the "
                  "current working directory '%s'. This is fragile, because if bitconnect is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if bitconnect is started while in a temporary directory.\n",
...
#ifdef ENABLE_CHECKPOINTS
                // bitconnect: initialize synchronized checkpoint
                if (!fReindex && !WriteSyncCheckpoint(chainparams.GenesisBlock().GetHash()))
                    return error("LoadBlockIndex() : failed to init sync checkpoint");

                // bitconnect: if checkpoint master key changed must reset sync-checkpoint
