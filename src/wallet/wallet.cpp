...
// bitconnect: optional setting to unlock wallet for block minting only;
...
//    return n ? n : nTimeReceived;
    ...
    // bitconnect: we still have the timestamp, so use it to avoid confusion
    return tx->nTime;
}
...
// bitconnect: total coins staked (non-spendable until maturity)
...

    {
        ...
        for (const auto& entry : mapWallet)
        {
            ...
            if (nSpendTime > 0 && pcoin->tx->nTime > nSpendTime)
                continue;  // bitconnect: timestamp must not exceed spend time
                                ...
                                int& nChangePosInOut, std::string& strFailReason, const CCoinControl& coin_control, bool sign)
{
    ...
    txNew.nTime = wtxNew.tx->nTime;  // bitconnect: set time
...
//                }
                ...
                // bitconnect: sub-cent change is moved to fee
...
// bitconnect: create coin stake transaction
