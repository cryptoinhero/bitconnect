    ...
    platformStyle(_platformStyle)
{
    ...
    progressBar->setStyleSheet("QProgressBar { background-color: #8C8C8C; text-align: center; color: white; border: 1px solid #4b4b4b; } QProgressBar::chunk { background: #f7931a; margin: 0px; }");
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Bitconnect address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and bitconnect: URIs)"));
...
//    connect(multisigAction, SIGNAL(triggered()), this, SLOT(gotoMultisigPage()));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Bitconnect addresses to prove you own them"));
    verifyMessageAction = new QAction(QIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Bitconnect addresses"));
    ...
    openAction->setStatusTip(tr("Open a bitconnect: URI or payment request"));

    showHelpMessageAction = new QAction(QIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Bitconnect command-line options").arg(tr(PACKAGE_NAME)));

    openChatroomAction = new QAction(QIcon(":/icons/bitconnect"), tr("&Chatroom"), this);
    openChatroomAction->setStatusTip(tr("Open the Bitconnect Discord in a web browser."));

    openForumAction = new QAction(QIcon(":/icons/bitconnect"), tr("&Forum"), this);
    openForumAction->setStatusTip(tr("Open https://talk.bitconnect.com in a web browser."));
...
void BitcoinGUI::openChatroom() {
    QDesktopServices::openUrl(QUrl("https://discord.gg/bitconnect"));
}

void BitcoinGUI::openForum() {
    QDesktopServices::openUrl(QUrl("https://talk.bitconnect.com"));
}
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Bitconnect network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
{
    QString strTitle = tr("Bitconnect"); // default title
...
void BitcoinGUI::subscribeToCoreSignals()
{
    ...
    uiInterface.ThreadSafeMessageBox.connect(boost::bind(ThreadSafeMessageBox, this,boost::placeholders::_1,boost::placeholders::_2,_3));
    uiInterface.ThreadSafeQuestion.connect(boost::bind(ThreadSafeMessageBox, this,boost::placeholders::_1,boost::placeholders::_3,_4));
}

void BitcoinGUI::unsubscribeFromCoreSignals()
{
    // Disconnect signals from client
    uiInterface.ThreadSafeMessageBox.disconnect(boost::bind(ThreadSafeMessageBox, this,boost::placeholders::_1,boost::placeholders::_2,_3));
    uiInterface.ThreadSafeQuestion.disconnect(boost::bind(ThreadSafeMessageBox, this,boost::placeholders::_1,boost::placeholders::_3,_4));
}
