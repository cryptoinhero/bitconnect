...
#if defined(NDEBUG)
# error "Bitconnect cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Bitconnect Signed Message:\n";
                              ...
                              bool bypass_limits, std::vector<COutPoint>& coins_to_uncache)
{
...

    {
        ...
        CAmount nFees = 0;
        uint64_t coinage = 0;
        if (!Consensus::CheckTxInputs(tx, state, view, GetSpendHeight(view), nFees, coinage, chainparams.GetConsensus())) {
        ...
        // bitconnect: if transaction is after version 0.8 fork, verify SCRIPT_VERIFY_LOW_S
...
// bitconnect: miner's coin stake is rewarded based on coin age spent (coin-days)
int64_t GetProofOfStakeReward(int64_t nCoinAge)
{
    ...
    int64_t nBestHeight = chainActive.Tip()->nHeight;
...
{
    if (fUpdateUI)
        uiInterface.NotifyAlertChanged(uint256(), CT_UPDATED); // bitconnect: we are using arguments that will have no effects in updateAlert()
...
int ApplyTxInUndo(Coin&& undo, CCoinsViewCache& view, const COutPoint& out)
{
    ...
    if (undo.nHeight == 0) {
        ...
        if (!alternate.IsSpent()) {
            ...
            undo.fCoinStake = alternate.fCoinStake; // bitconnect
            undo.nTime = alternate.nTime;           // bitconnect
        } else {
...
void ThreadScriptCheck() {
    RenameThread("bitconnect-scriptch");
...
bool CheckReward(const CBlock& block, CValidationState& state, const CCoinsViewCache& inputs, int nHeight, const Consensus::Params& consensusParams, CAmount nFees, uint64_t nCoinAge, CAmount nActualStakeReward)
{
    size_t offset = block.IsProofOfStake() ? 1 : 0;

    if (block.IsProofOfWork()){
        int64_t nReward = nFees + GetProofOfWorkReward(nHeight);
        if (block.vtx[offset]->GetValueOut() > nReward)
            return state.DoS(50, false, REJECT_INVALID, "bad-cb-amount", false,
                    strprintf("CheckBlock() : coinbase reward exceeded %s > %s",
                       FormatMoney(block.vtx[offset]->GetValueOut()),
                       FormatMoney(nReward)));
    } 
    if (block.IsProofOfStake()) {
        ...
        if(tx.IsCoinStake())
        {
            // bitconnect: coin stake tx earns reward instead of paying fee
            CAmount nStakeReward = GetProofOfStakeReward(nCoinAge);
            if (nActualStakeReward > nStakeReward + nFees)
                return state.DoS(100, false, REJECT_INVALID, "bad-txns-coinstake-too-large", false, strprintf("nFees = %d, nActualStakeReward = %s, nStakeReward = %s", FormatMoney(nFees), FormatMoney(nActualStakeReward), FormatMoney(nStakeReward)));
        }
    }
    return true;
}
// These checks can only be done when all previous block have been added.
bool BitconnectContextualBlockChecks(const CBlock& block, CValidationState& state, CBlockIndex* pindex, bool fJustCheck)
{
    uint256 hashProofOfStake = uint256();
    // bitconnect: verify hash target and signature of coinstake tx
    ...
    const CChainParams& chainparams = Params();
    if (block.IsProofOfWork() && block.GetHash() != chainparams.GetConsensus().hashGenesisBlock) {
        hashProofOfStake = block.GetHash();
    }
    // bitconnect: compute stake entropy bit for stake modifier
    unsigned int nEntropyBit = GetStakeEntropyBit(block);

    // bitconnect: compute stake modifier
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    if (pindex->nStakeModifier == 0 && pindex->nStakeModifierChecksum == 0 && !BitconnectContextualBlockChecks(block, state, pindex, fJustCheck))
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        else
        {
            CAmount txfee = 0;
            uint64_t coinage = 0;
            if (!Consensus::CheckTxInputs(tx, state, view, pindex->nHeight, txfee, coinage, chainparams.GetConsensus())) {
                return error("%s: Consensus::CheckTxInputs: %s, %s", __func__, tx.GetHash().ToString(), FormatStateMessage(state));
            }
            nCoinAge += coinage;
    ...
    if (!CheckReward(block, state, view, pindex->nHeight, chainparams.GetConsensus(), nFees, nCoinAge, nActualStakeReward))
        return state.Invalid(false, REJECT_INVALID, "block-reward-invalid");
    ...
    // bitconnect: track money supply and mint amount info
    pindex->nMint = nValueOut - nValueIn + nFees;
    pindex->nMoneySupply = (pindex->pprev? pindex->pprev->nMoneySupply : 0) + nValueOut - nValueIn;

    // bitconnect: fees are not collected by miners as in bitcoin
    // bitconnect: fees are destroyed to compensate the entire network
    if (gArgs.GetBoolArg("-printcreation", false))
...
public:
    explicit ConnectTrace(CTxMemPool &_pool) : blocksConnected(1), pool(_pool) {
        pool.NotifyEntryRemoved.connect(boost::bind(&ConnectTrace::NotifyEntryRemoved, this,boost::placeholders::_1,_2));
    }

    ~ConnectTrace() {
        pool.NotifyEntryRemoved.disconnect(boost::bind(&ConnectTrace::NotifyEntryRemoved, this,boost::placeholders::_1,_2));
    }
...
CBlockIndex* CChainState::AddToBlockIndex(const CBlockHeader& block, bool fSetAsProofOfstake)
{
    ...
    if (fSetAsProofOfstake || pindexNew->nHeight > params.nLastPOWBlock)
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSignature)
{
    ...
    // bitconnect: only the second transaction can be the optional coinstake
    for (unsigned int i = 2; i < block.vtx.size(); i++)
        if (block.vtx[i]->IsCoinStake())
            return state.DoS(100, false, REJECT_INVALID, "bad-cs-missing", false, "coinstake in wrong position");

    // bitconnect: first coinbase output should be empty if proof-of-stake block
    ...
    for (const auto& tx : block.vtx)
    {
        ...
        // bitconnect: check transaction timestamp
    ...
    // bitconnect: check block signature
...
#ifdef ENABLE_CHECKPOINTS
    // bitconnect: check that the block satisfies synchronized checkpoint
...
bool CChainState::AcceptBlockHeader(const CBlockHeader& block, bool fProofOfStake, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fOldClient)
{
    ...
    if (hash != chainparams.GetConsensus().hashGenesisBlock) {
        ...
        // bitconnect: Don't reject in case of old clients. Change our assumption instead.
...
//    // bitconnect: ask for pending sync-checkpoint if any
//    if (!IsInitialBlockDownload())
...
bool CChainState::AcceptBlock(const std::shared_ptr<const CBlock>& pblock, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fRequested, const CDiskBlockPos* dbp, bool* fNewBlock, bool fCheckPoS)
{
    ...
    // bitconnect: we should only accept blocks that can be connected to a prev block with validated PoS
    ...
    // bitconnect: check PoS
    if (fCheckPoS && !BitconnectContextualBlockChecks(block, state, pindex, false)) {
        pindex->nStatus |= BLOCK_FAILED_VALID;
        setDirtyBlockIndex.insert(pindex);
...
#ifdef ENABLE_CHECKPOINTS
    // bitconnect: check pending sync-checkpoint
...
#ifdef ENABLE_CHECKPOINTS
    // bitconnect: if responsible for sync-checkpoint send it
...
bool CChainState::LoadBlockIndex(const Consensus::Params& consensus_params, CBlockTreeDB& blocktree)
{
    ...
    for (const std::pair<int, CBlockIndex*>& item : vSortedByHeight)
    {
        ...
        // bitconnect: calculate stake modifier checksum
...
#ifdef ENABLE_CHECKPOINTS
    // bitconnect: load hashSyncCheckpoint
...
bool LoadBlockIndex(const CChainParams& chainparams)
{
    ...
    if (needs_init) {
        ...
        fTxIndex = true; // bitconnect: txindex is always enabled
...
// bitconnect: total coin age spent in transaction, in the unit of coin-days.
